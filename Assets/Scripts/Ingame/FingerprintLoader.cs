...
public class FingerprintLoader
{
    ...
    Dictionary<string, List<int>> fingerprintSpriteTable = null;

    public FingerprintLoader()
    {
        fingerprintSpriteTable = new Dictionary<string, List<int>> {
            { "보통궁상문", new List<int> { 0, 1, 2, 3, 4 } },
            { "돌기궁상문", new List<int> { 6, 7, 8, 9, 10 } },
            { "와상문", new List<int> { 12, 13, 14, 15, 16 } },
            { "좌제상문", new List<int> { 18, 19, 20, 21, 22 } },
            { "우제상문", new List<int> { 24, 25, 26, 27, 28 } },
            { "변태문", new List<int> { 30, 31, 32, 33, 34 } }
        };
    }
    ...
    **/
    public List<(string, int)> CreateFingerprintSet (
    ...
    List<string> incluedFingerprintTypes = null)
    {
        ...
        List<(string, int)> newFingerprintInfo = new List<(string, int)>();
        ...
        for (int createdFingerprint = 0; createdFingerprint < setSize; createdFingerprint++)
        {
            ...
            /** 지문 코드를 얻음 **/
            List<int> fingerprintSpriteList = fingerprintSpriteTable[newFingerprintType];
            int newFingerprintSpriteCode = fingerprintSpriteList[Random.Range(0, fingerprintSpriteList.Count)];
